<!DOCTYPE html>
<head>
<title>B2B Movies</title>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
<meta name="viewport" content="width=device-width, initial-scale=1">
<link rel="stylesheet" href="css/style.css" type="text/css" media="all" /><!-- link to style.css -->
<link href="showYtVideo.css" rel="stylesheet" type="text/css"><!-- link to showYtVideo.css -->

</head>
<body>
<!-- START PAGE SOURCE -->
<div id="shell">
  <div id="header">
    <h1 id="logo"><a href="index.php">B2B Movies</a></h1>
    
    <div id="navigation">
      <ul>
        <li><a href="index.php" href="#">HOME</a></li>
        <li><a href="news.php">NEWS</a></li>
        <li><a href="inthreater.php">IN THEATERS</a></li>
        <li><a href="comingsoon.php">COMING SOON</a></li>
        <li><a href="bookticket.php">BOOK TICKET</a></li>
		<li><a href="contact.php">CONTACT</a></li>
      </ul>
    </div>
	
<br><br>
<div class="slideshow-container">
<br><br>
<div class="mySlidesfade1">
  <img src="css/images/jokerr.jpg" height="300" width="100%">
  <div class="text">
				<ul>
                            <li>Movie name : Joker</li>
                            <li>Director : Todd Philips</li>
                            <li>Genre : Crime/Psychological Thriller movie</li>
							<li>Movie Venue: QFX,Trademall</li>
							<li>Time :12:00-3:00 <script></script></li>
                           <li><a href="https://in.bookmyshow.com/mumbai/movies/joker/ET00100071">BOOK TICKET</a></li><!-- link to book ticket page -->
				</ul>
</div>
</div>

<div class="mySlides fade">
  <img src="css/images/housie.jpg" height="300" width="100%">
  <div class="text">
				<ul>
                            <li>Movie name : Housefull 4</li>
                            <li>Director : Farhad Samji</li>
                            <li>Genre : Comedy</li>
							<li>Movie Venue: QFX,Trademall</li>
							<li>Time :12:00-3:00 <script></script></li>
                            <li><a href="https://in.bookmyshow.com/mumbai/movies/housefull-4/ET00064161">BOOK TICKET</a></li><!-- link to book ticket page -->
				</ul>
				
</div>
</div>

<div class="mySlides fade">
  <img src="css/images/chhhi.jpg" height="300" width="100%">
  <div class="text">
						<ul>
                            <li>Movie name : Chhichhore</li>
                            <li>Director : 	Sandeep Tiwari</li>
                            <li>Genere : Comedy/Drama</li>
							<li>Movie Venue: QFX,Trademall</li>
							<li>Time:4:00-7:00<script></script></li>
                           <li><a href="https://in.bookmyshow.com/mumbai/movies/chhichhore/ET00085935">BOOK TICKET</a></li><!-- link to book ticket page -->
						</ul>
  </div>
</div>

<div class="mySlides fade">
  <img src="css/images/warr.jpg" height="300" width="100%">
  <div class="text">
  				<ul>
                            <li>Movie name : War</li>
                            <li>Director : Siddharth Anand</li>
                            <li>Genere : Action Movie</li>
							<li>Movie Venue: QFX,Trademall</li>
							<li>Time :3:00-6:00<script></script></li>
							<li><a href="https://in.bookmyshow.com/mumbai/movies/war/ET00107162">BOOK TICKET</a></li><!-- link to book ticket page -->
				</ul>
  </div>
</div>

</div>
<br>


  <div id="footer">
  
  <tr>
				<td colspan="4" height="20">
				</td>
			</tr>
		</table>
		<table border="0px" cellspacing="0" cellpadding="0" width="960px" height="100" background:url('images/body-bg.gif');><!-- table for link to social medias -->
			<tr>
				<td>
				<center>
				<a href="http://www.facebook.com"><img src="css/social icons/facebook.png" height="30" width="30"></a>
				<a href="http://www.twitter.com"><img src="css/social icons/twitter.png" height="30" width="30"></a>
				<a href="http://www.youtube.com"><img src="css/social icons/youtube.png" height="30" width="30"></a>
				<a href="http://www.instagram.com"><img src="css/social icons/instagram.png" height="30" width="30"></a>
				<p>Copy Right &copy; Protected</p>
				</center>
				</td>
			</tr>
		</table>
  
    <p <a href="#">B2B Movies</a> - All Rights Reserved</p>
    <p class="rf">Design by <a href="http://B2B Movies.com/">Pooja & Neha</a></p>
    <div style="clear:both;"></div>
  </div>
</div>

<script src="http://code.jquery.com/jquery-1.11.3.min.js"></script>
<script src="js/jquery.showYtVideo.js"></script>
<script>

 jQuery(document).ready(function ($) {
            $('.show-video').on('click', function () {
			     var id = this.id;

                $.showYtVideo({
                    videoId: id
                });
            });
        });
</script>

<!-- END PAGE SOURCE -->
</body>
</html>